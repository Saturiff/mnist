...
void  Data::ResetData()
{
	offsetOfImage = offsetOfLabel = 0;
	delete[] label, image;
	label = new byte[1];
	image = new byte[NUMBER_OF_TOTAL_PIXEL];
}
...
void Data::ReadNextImage(const char fileName[])
{
	FILE* imageFile = fopen(fileName, "rb");
		...
		fread(image, sizeof(byte), NUMBER_OF_TOTAL_PIXEL, imageFile);
	fclose(imageFile);
	offsetOfImage += NUMBER_OF_TOTAL_PIXEL;
}
